...
// ---- Search bar (di-debounce biar nggak render ulang tiap huruf, lebih smooth) ----
...
const copyGeneratedLabel = document.getElementById("copyGeneratedLabel");
copyGeneratedBtn.addEventListener("click", () => {
  navigator.clipboard.writeText(generatedCode.textContent).then(() => {
    copyGeneratedLabel.textContent = "Tersalin!";
    setTimeout(() => { copyGeneratedLabel.textContent = "Copy Kode"; }, 1600);
  });
});
...
const codePanelHint  = document.getElementById("codePanelHint");

function copyScriptCode(){
  navigator.clipboard.writeText(scriptCodeText.textContent).then(() => {
    copyCodeLabel.textContent = "Tersalin!";
    codePanelHint.textContent = "tersalin ✓";
    setTimeout(() => {
      copyCodeLabel.textContent = "Copy Script";
      codePanelHint.textContent = "siap copy ↓";
    }, 1600);
  }).catch(() => {
    copyCodeLabel.textContent = "Gagal, salin manual";
    setTimeout(() => { copyCodeLabel.textContent = "Copy Script"; }, 1600);
  });
}

copyCodeBtn.addEventListener("click", copyScriptCode);
codePanel.addEventListener("click", copyScriptCode);
codePanel.addEventListener("keydown", (e) => {
  if(e.key === "Enter" || e.key === " "){ e.preventDefault(); copyScriptCode(); }
});
